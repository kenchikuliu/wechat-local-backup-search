...
def _tasklist_for_image(image_name):
    import subprocess
...
def get_pids():
    ...
    WeChat 4.x Windows 进程名通常是 Weixin.exe；部分 3.x/旧安装仍是
    WeChat.exe。这里同时兼容，避免误报“微信未运行”。
    """
    pids = []
    for image_name in ("Weixin.exe", "WeChat.exe"):
        r = _tasklist_for_image(image_name)
        for line in r.stdout.strip().split('\n'):
            ...
            if len(p) >= 5:
                ...
                pids.append((pid, mem, image_name))
    if not pids:
        raise RuntimeError("未检测到 Weixin.exe / WeChat.exe")
    pids.sort(key=lambda x: x[1], reverse=True)
    for pid, mem, image_name in pids:
        print(f"[+] {image_name} PID={pid} ({mem // 1024}MB)")
